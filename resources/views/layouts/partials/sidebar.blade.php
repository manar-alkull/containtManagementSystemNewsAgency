<!-- Left side column. contains the logo and sidebar -->
<aside class="main-sidebar" id="toArabic">

    <!-- sidebar: style can be found in sidebar.less -->
    <section class="sidebar">

        <!-- Sidebar user panel (optional) -->
        @if (! Auth::guest())
            <div class="user-panel">
                <div class="pull-left image">
                    <img src="{{asset('/img/user2-160x160.jpg')}}" class="img-circle" alt="User Image" />
                </div>
                <div class="pull-left info">
                    <p>{{ Auth::user()->name }}</p>
                    <!-- Status -->
                    <a href="#"><i class="fa fa-circle text-success"></i> {{ trans('adminlte_lang::message.online') }}</a>
                </div>
            </div>
        @endif

        <!-- search form (Optional) -->
        <form action="#" method="get" class="sidebar-form">
            <div class="input-group">
                <input type="text" name="q" class="form-control" placeholder="{{ trans('adminlte_lang::message.search') }}..."/>
              <span class="input-group-btn">
                <button type='submit' name='search' id='search-btn' class="btn btn-flat"><i class="fa fa-search"></i></button>
              </span>
            </div>
        </form>
        <!-- /.search form -->

        <!-- Sidebar Menu -->
{{--        {{ trans('adminlte_lang::message.home') }}--}}
        <ul class="sidebar-menu">
            <li class="header">{{ trans('adminlte_lang::message.header') }}</li>
            <!-- Optionally, you can add icons to the links -->
            {{--<li class=""><a href="{{ url('home') }}"><i class='fa fa-link'></i> <span>{{ trans('adminlte_lang::message.home') }}</span></a></li>--}}
            @if(Auth::user()->role == 1)
                <li><a href="{{url("/menu")}}"><i class='fa fa-link'></i> <span>{{ "Menu" }}</span></a></li>
                <li><a href="{{url("/language")}}"><i class='fa fa-link'></i> <span>{{ "Language" }}</span></a></li>
                <li><a href="{{url("/dictionary")}}"><i class='fa fa-link'></i> <span>{{ "Dictionary" }}</span></a></li>
                <li><a href="{{url("/category")}}"><i class='fa fa-link'></i> <span>{{ "Category" }}</span></a></li>
                <li><a href="{{url("/permission")}}"><i class='fa fa-link'></i> <span>{{ "permission" }}</span></a></li>
                <li><a href="{{url("/users")}}"><i class='fa fa-link'></i> <span>{{ "Users" }}</span></a></li>
            @endif
            <li><a href="{{url("/category")}}"><i class='fa fa-link'></i> <span>{{ "Category" }}</span></a></li>
            <li class="treeview">
                <a href="#"><i class='fa fa-link'></i> <span>{{ trans('adminlte_lang::message.multilevel') }}</span> <i class="fa fa-angle-left pull-right"></i></a>
                <ul class="treeview-menu">
                    <li><a href="#">{{ trans('adminlte_lang::message.linklevel2') }}</a></li>
                    <li><a href="#">{{ trans('adminlte_lang::message.linklevel2') }}</a></li>
                </ul>
            </li>
        </ul><!-- /.sidebar-menu -->
    </section>
    <!-- /.sidebar -->
</aside>
